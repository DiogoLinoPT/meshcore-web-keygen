...
for n in raw_nodes:
    ...
    # Spain/Iberia Bounding Box
    if 27.0 <= lat <= 44.0 and -19.0 <= lon <= 4.5:
...
out_path = os.path.join(os.path.dirname(__file__), 'nodes_es.json')
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(out_data, f, ensure_ascii=False, separators=(',', ':'))

print(f"Success! Filtered down to {len(filtered_nodes)} nodes for Spain.")
